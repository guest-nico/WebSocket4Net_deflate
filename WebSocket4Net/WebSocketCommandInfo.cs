...
namespace WebSocket4Net
{
    public class WebSocketCommandInfo : ICommandInfo
    {
        ...
        public WebSocketCommandInfo(string key, string text, byte[] d)
        {
            Key = key;
            Text = text;
            Data = d;
        }
        public WebSocketCommandInfo(string key, string text, string a)
        {
            Key = key;
            Text = text;
            //Data = d;
            ...
            if (frame.OpCode == OpCode.Close)
            {
                if (length >= 2)
                {
                    ...
                    if (length > 2)
                    {
                        ...
                        Data = frame.InnerData.ToArrayData(offset + 2, length - 2);
                        //d.CopyTo(Data, offset + 2, 0, length - 2);
                    }
                    else
                    {
                        Text = string.Empty;
