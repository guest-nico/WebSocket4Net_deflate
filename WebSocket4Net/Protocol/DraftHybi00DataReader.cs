        ...
        public override WebSocketCommandInfo GetCommandInfo(byte[] readBuffer, int offset, int length, out int left)
        {
                ...
                for (i = offset + skipByteCount; i < offset + length; i++)
                {
                    if (readBuffer[i] == lookForByte)
                    {
                        ...
                        if (BufferSegments.Count <= 0)
                        {
                        	...
                        	Array.Copy(readBuffer, offset + skipByteCount, d, 0, i - offset - skipByteCount);
                            var commandInfo = new WebSocketCommandInfo(OpCode.Text.ToString(), Encoding.UTF8.GetString(readBuffer, offset + skipByteCount, i - offset - skipByteCount), d);
                            Reset(false);
                            return commandInfo;
                        }
                        else
                        {
                            ...
                            var commandInfo = new WebSocketCommandInfo(OpCode.Text.ToString(), BufferSegments.Decode(Encoding.UTF8), d);
                            Reset(true);
                            return commandInfo;
                        }
                    }
            ...
            else//10000000: Collect protocol data by length
            {
                ...
                else
                {
                    ...
                    {
                    	byte[] d = new byte[100000];
                        Array.Copy(readBuffer, offset + skipByteCount, d, 0, requiredSize);
                        
                        var commandInfo = new WebSocketCommandInfo(OpCode.Text.ToString(), Encoding.UTF8.GetString(readBuffer, offset + skipByteCount, requiredSize), d);
                        Reset(false);
                        return commandInfo;
                    }
                    else
